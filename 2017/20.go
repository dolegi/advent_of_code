...
//p=< 4,0,0>, v=< 0,0,0>, a=<-2,0,0>`
	input = `p=<2366,784,-597>, v=<-12,-41,50>, a=<-5,1,-2>
...
func getCoord(str string) coord {
	...
	for _, c := range strings.Split(str, ",") {
		i, _ := strconv.Atoi(strings.Replace(c, " ", "", -1))
		coordArr = append(coordArr, i)
	}
	return coord{coordArr[0], coordArr[1], coordArr[2]}
}
...
func getParticles(in string) (particles []particle) {
	for _, p := range strings.Split(input, "\n") {
		...
		part := particle{}
		part.p = getCoord(strings.Split(fields[0], "<")[1])
		part.v = getCoord(strings.Split(fields[1], "<")[1])
		part.a = getCoord(strings.Split(fields[2], "<")[1])
...
func absCoord(c coord) int {
	return int(math.Abs(float64(c.x))) +
		int(math.Abs(float64(c.y))) +
		int(math.Abs(float64(c.z)))
}

func getSlowestAcc(particles []particle) int {
	slowestAcc := absCoord(particles[0].a)
	for _, p := range particles {
		if absCoord(p.a) < slowestAcc {
			slowestAcc = absCoord(p.a)
		}
	}
	return slowestAcc
}

func getClosestIndex(particles []particle) int {
	slowestAcc := getSlowestAcc(particles)
	slowest := []int{}

	for i, p := range particles {
		if absCoord(p.a) == slowestAcc {
			slowest = append(slowest, i)
		}
	}
	closestIndex := slowest[0]
	for _, i := range slowest {
		if absCoord(particles[i].p) < absCoord(particles[closestIndex].p) {
			closestIndex = i
		}
	}
	return closestIndex
