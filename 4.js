...
lines.sort()
lines.forEach(line => {
  const guardIdMatch = line.match(/(?<=#)\d+/)
  if (guardIdMatch) {
    guardId = guardIdMatch[0]
    return
  }
  ...
  if (/falls asleep/.test(line)) {
    ...
    for (let i = start; i < time; i++) {
      if (!timings[guardId][i]) {
        timings[guardId][i] = 0
      }
      timings[guardId][i] += 1
    }
...
let sleepyGuardId = 0
Object.entries(timings).reduce((mostSleep, [guard, time]) => {
  if (time.reduce((acc, x) => acc + x) > mostSleep) {
    sleepyGuardId = guard
  }
}, 0)
let common
let bigCount = 0
timings[sleepyGuardId].forEach((t, i) => {
  if (t > bigCount) {
    common = i
    bigCount = t
  }
})
console.log(sleepyGuardId)
console.log(sleepyGuardId*common)


Object.entries(timings).forEach(([guard, time]) => {
  let common
  let bigCount = 0
  time.forEach((t, i) => {
    if (t > bigCount) {
  ...
  console.log(
    guard,
    '\t',
    time.reduce((acc, x) => acc + x),
    '\t',
    Math.max(...time.filter(x => x !== undefined)),
    '\t',
    common,
    '\t',
    guard * common
